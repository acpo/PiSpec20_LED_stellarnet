...
# GET SPECTROMETER CONFIGURATION
    configurl = deviceurl+'/'+device[0]+'/config'   #assumes first enumerated device is the one you want
    ...
    IntTime = 50    #sets an arbitrary but often useful integration time
    Averages = 1    #sets single acquisition
    headers = {
...
# GET INITIAL SPECTRUM AND SETUP VARIABLES
    ...
    xmax = np.around(max(x), decimals=2)        #x-axis (wavelength) range
    ymin = np.around(min(data), decimals=2)
    ymax = np.around(max(data), decimals=2)     #y-axis extents
