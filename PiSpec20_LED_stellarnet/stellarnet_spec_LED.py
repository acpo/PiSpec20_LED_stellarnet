# version May 2024
# ## pigpio daemon must be running first
...
#Spectrometer initial setup
        ...
        self.LEDpin = 18     #BCM pin 18 is Physical pin 12 on the RPi
        self.LEDfrequency = 2000    # 2000 Hz is suitable for an integration time of 2 ms or longer
        self.LEDdutycycle = 50000    #50000 is 5%, scale is zero to 1 million, 0 is off
        self.LEDstate = 0    # LED off initially
...
##        self.button_saveFile.bind('<ButtonRelease-1>', self.saveFile)
        ...
        self.labelLED = tk.Label(self.menu_left_upper, text="LED power (1 to 100%)")
        self.labelLED.grid(column=0, row=10, pady=2)
        self.entryLED = tk.Entry(self.menu_left_upper, width= '5')
        self.entryLED.grid(column=1, row=10, pady=2)
        self.entryLED.insert(0, self.LEDdutycycle / 10000)
        self.entryLED.bind('<Return>', self.entryLED_return) and self.entryLED.bind('<Tab>', self.entryLED_return)
        self.buttonLED = tk.Button(self.menu_left_upper, text='LED on / off', background = 'light grey')
        self.buttonLED.grid(column=0, row=11, pady=2)
        self.buttonLED.bind('<ButtonRelease-1>', self.LEDstatebutton)
...
## start Shared (OO/Stellarnet) defs
    ...
    def ButtonQuit(self):
        gpio.hardware_PWM(self.LEDpin, self.LEDfrequency, 0) # turn off LED
        self.StopCode = True
        App.destroy(self)
        tk.Frame.quit(self)
        exit(0)

    def LEDstatebutton(self, event):
        if self.LEDstate == 1:
            self.LEDstate = 0
            gpio.hardware_PWM(self.LEDpin, self.LEDfrequency, 0)
            self.buttonLED.configure(background = 'light grey')
        else:
            self.LEDstate = 1
            gpio.hardware_PWM(self.LEDpin, self.LEDfrequency, self.LEDdutycycle)
    ...
    def entryLED_return(self, event):
        ...
        try:
            ...
            if 0 < LEDdutycycletemp <= 100:
                self.LEDdutycycle = int(10000 * LEDdutycycletemp)    #factor of 10000 translates % to PWM units
